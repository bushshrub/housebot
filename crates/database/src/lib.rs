...
mod tests {
    ...
    #[test]
    fn purge_runs_immediately_after_the_ledger_bootstrap() {
        let purge = MIGRATIONS
            .iter()
            .position(|(version, _)| *version == "001_purge_all_data")
            .expect("purge migration must exist");
        assert_eq!(
            purge, 1,
    ...
    #[test]
    fn purge_drops_the_schema_and_restores_the_ledger() {
        let (_, sql) = MIGRATIONS
            .iter()
            .find(|(version, _)| *version == "001_purge_all_data")
            .expect("purge migration must exist");
        assert!(sql.contains("DROP SCHEMA public CASCADE"));
        assert!(sql.contains("CREATE SCHEMA public"));
        assert!(sql.contains("CREATE TABLE IF NOT EXISTS schema_migrations"));
        assert!(sql.contains("'000_create_schema_migrations'"));
    }
}
